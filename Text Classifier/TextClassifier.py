...
predicted_nb_gv = gs_clf_nb.predict(X_test)
...
text_clf_svm = Pipeline([('vect', CountVectorizer()),
                        ...
                        ('clf-svm', SGDClassifier(loss='hinge', penalty='l2',
                                    alpha=1e-3, max_iter=5, random_state=42, class_weight = 'balanced')),
                        ])
...
text_clf_log = Pipeline([('vect', CountVectorizer()),
                        ('tfidf', TfidfTransformer()),
                        ('clf-log', LogisticRegression(multi_class='multinomial', solver='newton-cg', class_weight = 'balanced')),
                        ])
...
# Specifing a 5 fold Cross Validation
...
for train_index, test_index in kf.split(summaries, genres):
	...
	# Spliting the dataset based on the test and train index specified by the fold
...
pickle.dump(text_clf_log_cv, open("logReg.pkl", "wb"))
